...
/** Wait for a media element event, giving up after `timeoutMs` — a stalled
 * seek on a slow/corrupt file must reject instead of hanging the caller's
 * busy state forever. */
function wait(el: HTMLVideoElement, ev: string, timeoutMs = 8000) {
  return new Promise<void>((resolve, reject) => {
    ...
    timer = setTimeout(() => {
      el.removeEventListener(ev, ok);
      el.removeEventListener("error", fail);
      reject(new Error(`video ${ev} timed out`));
    }, timeoutMs);
